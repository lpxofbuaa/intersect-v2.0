...
bool GeometryFactory::point_on_line(Point *p, Line &l) {
	// ax + by + c = 0
	return equals((double)(l.a * p->x + l.b * p->y + l.c), 0.0);
}

bool GeometryFactory::point_on_circle(Point *p, Circle &c) {
	// (x - x0)^2 + (y - y0)^2 == r^2
...
void GeometryFactory::removeLine(Line &l) {
	...
	for (LineMap::iterator i = lines.begin(); i != lines.end(); ++i) {
		for (IdLineMap::iterator j = i->second.begin(); j != i->second.end(); ++j) {
			line_line_intersect(j->second, l, false);
		}
	}

	for (CircleSet::iterator i = circles.begin(); i != circles.end(); ++i) {
		Circle c2 = *i;
		line_circle_intersect(l, c2, false);
	}
}
...
void GeometryFactory::removeCircle(Circle &c) {
	...
	for (LineMap::iterator i = lines.begin(); i != lines.end(); ++i) {
		for (IdLineMap::iterator j = i->second.begin(); j != i->second.end(); ++j) {
			line_circle_intersect(j->second, c, false);
		}
	}

	for (CircleSet::iterator i = circles.begin(); i != circles.end(); ++i) {
		Circle c2 = *i;
		circle_circle_intersect(c2, c, false);
	}
}
...
bool GeometryFactory::point_in_line_range(double x, double y, Line & l)
{
	switch (l.type)
	{
	case DOUBLE_INFINITE_LINE:
...
void GeometryFactory::line_line_intersect(Line &l1, Line &l2, bool increase) {
	...
	if (delta_k != 0) {
		...
		if (l1.x1 == l2.x1 && l1.y1 == l2.y1) {
			Point tmp((double)l1.x1, (double)l1.y1);
			if (increase) {
				increase_point(&tmp);
			}
			else {
			...
			Point tmp((double)l1.x2, (double)l1.y2);
			if (increase) {
				increase_point(&tmp);
			}
			else {
...
// todo : check 
void GeometryFactory::line_circle_intersect(Line &l, Circle &c, bool increase) {
	long long delta_y_2;
	if (l.b != 0) {
		// k exists
		...
		if (delta_y_2 > 0) {
			...
			if (point_in_line_range((double)l.x1, (double)c.b, l)) {
				Point tmp((double)l.x1, (double)c.b);
				if (increase) {
					increase_point(&tmp);
				}
				else {
...
// todo: check
void GeometryFactory::circle_circle_intersect(Circle &c1, Circle &c2, bool increase) {
	// get common line
	...
	if (b != 0) {
		...
		if (distance_2 < r_2_out && distance_2 > r_2_in) {
			...
			Point p1(res1_x, res1_y);
			Point p2(res2_x, res2_y);
		...
		if (distance_2 < r_2_out && distance_2 > r_2_in) {
			...
			Point p1((double)x1, res_y1);
			Point p2((double)x1, res_y2);
